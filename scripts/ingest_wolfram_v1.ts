#!/usr/bin/env bun
// =============================================================================
// scripts/ingest_wolfram_v1.ts — extract one capability TOML per per-function
// PDF in data/wolfram-v1/raw/contents/.
// =============================================================================
//
// Modes:
//   single — one capability:
//     bun scripts/ingest_wolfram_v1.ts \
...
//     bun scripts/ingest_wolfram_v1.ts --bulk [--max-n <N>]
//
// Pipeline per PDF:
...
const RAW      = join(ROOT, "data/wolfram-v1/raw/contents");
const LEDGER   = join(ROOT, "data/wolfram-v1/raw/sha256.sum");
const B8_HTML  = join(RAW, "B.8.html");
const CAPS_DIR = join(ROOT, "capabilities/wolfram-v1");
...
source_kind  = "scrape"
retrieved_at = "${today}"
local_path   = "data/wolfram-v1/raw/contents/${pdfFilename}"
sha256       = "${sha}"
`;
  ...
  return `# Auto-generated by scripts/ingest_wolfram_v1.ts on ${today}.
# Source: data/wolfram-v1/raw/contents/${pdfFilename}.
#
...
ingested_at = "${today}"
ingested_by = "scripts/ingest_wolfram_v1.ts"
...
if (has("--bulk")) {
  ...
  if (!nameArg && !pdfArg) {
    process.stderr.write("usage: bun scripts/ingest_wolfram_v1.ts (--name <Function> | --pdf <listN.pdf>) [--out <path>]\n");
    ...
    process.stderr.write("       bun scripts/ingest_wolfram_v1.ts --bulk [--max-n <N>]\n");
    process.exit(2);
  }
